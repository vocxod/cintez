...
use \OcProductToCategory;
use \OcProductToCategoryQuery;
...
/**
 * This class defines the structure of the 'oc_product_to_category' table.
 *
 *
 *
 ...
 *
 */
class OcProductToCategoryTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.OcProductToCategoryTableMap';
     ...
     */
    const TABLE_NAME = 'oc_product_to_category';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\OcProductToCategory';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'OcProductToCategory';
     ...
     */
    const COL_PRODUCT_ID = 'oc_product_to_category.product_id';
    ...
    {
        // attributes
        $this->setName('oc_product_to_category');
        $this->setPhpName('OcProductToCategory');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\OcProductToCategory');
        $this->setPackage('');
     ...
     * and findPk*() calls.
     *
     * @param \OcProductToCategory $obj A \OcProductToCategory object.
     ...
     *
     * @param mixed $value A \OcProductToCategory object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \OcProductToCategory) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \OcProductToCategory object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? OcProductToCategoryTableMap::CLASS_DEFAULT : OcProductToCategoryTableMap::OM_CLASS;
    }
     ...
     * @return array           (OcProductToCategory object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = OcProductToCategoryTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = OcProductToCategoryTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + OcProductToCategoryTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = OcProductToCategoryTableMap::OM_CLASS;
            /** @var OcProductToCategory $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            OcProductToCategoryTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = OcProductToCategoryTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = OcProductToCategoryTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(OcProductToCategoryTableMap::DATABASE_NAME)->getTable(OcProductToCategoryTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(OcProductToCategoryTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(OcProductToCategoryTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new OcProductToCategoryTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a OcProductToCategory or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or OcProductToCategory object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \OcProductToCategory) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(OcProductToCategoryTableMap::DATABASE_NAME);
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(OcProductToCategoryTableMap::COL_PRODUCT_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(OcProductToCategoryTableMap::COL_CATEGORY_ID, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = OcProductToCategoryQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            OcProductToCategoryTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                OcProductToCategoryTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return OcProductToCategoryQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a OcProductToCategory or Criteria object.
     *
     * @param mixed               $criteria Criteria or OcProductToCategory object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from OcProductToCategory object
        }
...
} // OcProductToCategoryTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
OcProductToCategoryTableMap::buildTableMap();
